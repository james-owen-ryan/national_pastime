...
class Adoption(Event):
    ...
    def __init__(self, subject, adoptive_parents):
        ...
        """
        super(Adoption, self).__init__(cosmos=subject.cosmos)
...
class Birth(Event):
    ...
    def __init__(self, mother, doctor):
        ...
        super(Birth, self).__init__(cosmos=mother.cosmos)
        ...
        try:
            mother.cosmos.birthdays[(self.month, self.day)].add(self.subject)
        except KeyError:
            mother.cosmos.birthdays[(self.month, self.day)] = {self.subject}
        self._name_baby()
    ...
    def _name_baby(self):
        ...
        """
        config = self.subject.cosmos.config
        baby = self.subject
        if (
    ...
    def _have_mother_potentially_quit_job(self):
        ...
        if not self.city.businesses_of_type('DayCare'):
            ...
            if random.random() < self.mother.cosmos.config.chance_new_mother_quits_job_even_if_day_care_in_town:
                self.mother.occupation.terminate(reason=self)

    def _remunerate(self):
        """Have parents pay hospital for services rendered."""
        config = self.mother.cosmos.config
...
class BusinessConstruction(Event):
    ...
    def __init__(self, subject, business, architect, demolition_that_preceded_this=None):
        ...
        super(BusinessConstruction, self).__init__(cosmos=subject.cosmos)
        self.subject = subject
...
class BusinessClosure(Event):
    ...
    def __init__(self, business, reason):
        """Initialize a Demolition object."""
        super(BusinessClosure, self).__init__(cosmos=business.city.cosmos)
        ...
        # Shut down operations of this specific holding company
        if business.__class__.__name__ == 'HoldingCompany':
            self._shut_down_holding_company()
    ...
    def _shut_down_holding_company(self):
        """Carry out specific tasks related to the closure of a holding company."""
        for company in self.business.holdings:
            company.held_by = None
        # If this holding company is shutting down due to its magnate dying or retiring,
        # have its held companies hire new owners
        if (not self.business.owner.person.alive) or self.business.owner.person.retired:
            for company in self.business.holdings:
                company.hire(occupation_of_need=company.owner.__class__, shift="day", to_replace=company.owner)
...
class Death(Event):
    ...
    def __init__(self, subject, mortician, cause_of_death):
        ...
        super(Death, self).__init__(cosmos=subject.cosmos)
        ...
        if subject.kids_at_home and not subject.spouse:
            for kid in subject.kids_at_home:
                kid.die(cause_of_death='Starvation')  # Duct tape
    ...
    def _vacate_job_position_of_the_deceased(self):
        """Vacate the deceased's job position(s), if any."""
        # Check exhaustively for unterminated occupations, since some people (magnates)
        # may have multiple active occupations, which means we can't simple check for
        # 'self.subject.occupation'
        for occupation in self.subject.occupations:
            if not occupation.terminus:
                occupation.terminate(reason=self)
...
class Demolition(Event):
    ...
    def __init__(self, building, demolition_company, reason=None):
        """Initialize a Demolition object."""
        super(Demolition, self).__init__(cosmos=building.city.cosmos)
    ...
    def _have_the_now_displaced_residents_move(self, house_or_apartment_unit):
        ...
        # First, have the displaced residents attempt to find a new home in
        # the same city
        if house_or_apartment_unit.owners:
            decision_maker = list(house_or_apartment_unit.owners)[0]
            forced_cohort = set()  # Will automatically use their nuclear family
        else:
            print "{} in {} has residents (e.g., {}), but not owners.".format(
                house_or_apartment_unit, list(house_or_apartment_unit.residents)[0].name,
                house_or_apartment_unit.city.name,
            )
            # TODO modify this once orphanages implemented
            decision_maker = house_or_apartment_unit.residents[0]
            # Need to manually set forced_cohort here, because who knows what's going on --
            # probably kids living alone, and thus we can't rely on their nuclear family
            # to be accurately inferred
            forced_cohort = set(house_or_apartment_unit.residents)
        home_they_will_move_into = decision_maker.secure_home(city=decision_maker.city)
        if home_they_will_move_into:
            decision_maker.move(new_home=home_they_will_move_into, reason=self, forced_cohort=forced_cohort)
        else:
            # If that fails, have them find a new city to move to; let them be desperate,
            # if they have to (this ensures that they will for sure find a home to move to)
            new_city_to_move_to = decision_maker.choose_new_city_to_move_to(desperate=True)
            decision_maker.move_to_new_city(city=new_city_to_move_to, reason=self, forced_cohort=forced_cohort)
...
class Departure(Event):
    """A departure by which someone leaves a city."""
    ...
    def __init__(self, subjects, reason):
        ...
        assert len({s.city for s in self.subjects}) == 1, (
            "{} live in different cities but are attempting to depart together."
        )
        for person in self.subjects:
            person.departures.append(self)
            person.city.residents.remove(person)
            person.city.departed.add(person)
            self._vacate_job_positions_of_the_departed()

    def __str__(self):
        """Return string representation."""
        return "Departure from {} by {} in {}".format(
            self.city, ", ".join(s.name for s in self.subjects), self.year
        )
    ...
    def _update_neighbor_attributes(self):
        ...
        for subject in self.subjects:
            ...
            subject.neighbors = set()

    def _vacate_job_positions_of_the_departed(self):
        """Vacate the departed's job position, if any."""
        # Something to watch for: I think elsewhere I've made it so that magnates will
        # always stay in the city that their holding company is based in, but if I haven't
        # in fact done this, there could be issues here where they cancel their existing
...
class Divorce(Event):
    ...
    def __init__(self, subjects, lawyer):
        ...
        super(Divorce, self).__init__(cosmos=subjects[0].cosmos)
        self.city = subjects[0].city
        self.subjects = subjects
        self.lawyer = lawyer
    ...
    def _update_divorcee_attributes(self):
        ...
        salience_change = (
            spouse1.cosmos.config.salience_increment_from_relationship_change['significant other'] +
            spouse1.cosmos.config.salience_increment_from_relationship_change['immediate family']
        )
    ...
    def _have_a_spouse_and_possibly_kids_change_name_back(self):
        ...
        config = self.subjects[0].cosmos.config
    ...
    def _decide_where_spouse_moving_out_will_live(spouse_who_will_move):
        ...
        # If that fails, have them find a new city to move to; let them be desperate,
        # if they have to (this ensures that they will for sure find a home to move to)
        home_in_this_city = spouse_who_will_move.secure_home(city=spouse_who_will_move.city)
        if home_in_this_city:
            return home_in_this_city
        else:
            new_city_to_move_to = spouse_who_will_move.choose_new_city_to_move_to(desperate=True)
            home_in_that_city = spouse_who_will_move.secure_home(city=new_city_to_move_to)
            return home_in_that_city
    ...
    def _move_spouse_and_possibly_kids_out(self, home_spouse_will_move_to, family_members_who_will_move):
        ...
        if home_spouse_will_move_to:
            ...
            # If that fails, have them find a new city to move to; let them be desperate,
            # if they have to (this ensures that they will for sure find a home to move to)
            parent_moving_out = max(family_members_who_will_move, key=lambda f: f.age)
            new_city_to_move_to = parent_moving_out.choose_new_city_to_move_to(desperate=True)
            parent_moving_out.move_to_new_city(
                city=new_city_to_move_to, reason=self, forced_cohort=family_members_who_will_move
            )
...
class Hiring(Event):
    ...
    def __init__(self, subject, company, occupation):
        ...
        super(Hiring, self).__init__(cosmos=subject.cosmos)
        self.subject = subject
...
class HomePurchase(Event):
    ...
    def __init__(self, subjects, home, realtor):
        ...
        super(HomePurchase, self).__init__(cosmos=subjects[0].cosmos)
        self.city = subjects[0].city
        self.subjects = subjects
...
class LayOff(Event):
    ...
    def __init__(self, subject, company, occupation):
        ...
        super(LayOff, self).__init__(cosmos=subject.cosmos)
        self.subject = subject
...
class Marriage(Event):
    ...
    def __init__(self, subjects):
        ...
        super(Marriage, self).__init__(cosmos=subjects[0].cosmos)
        self.city = subjects[0].city
        self.subjects = subjects
    ...
    def duration(self):
        ...
        if self.terminus:
            ...
            duration = self.subjects[0].cosmos.year-self.year
        return duration
    ...
    def _decide_where_newlyweds_will_live(self):
        ...
        if any(s for s in self.subjects if s in s.home.owners):
            ...
            home_in_this_city = self.subjects[0].secure_home(city=self.subjects[0].city)
            if home_in_this_city:
                return home_in_this_city
            else:
                new_city_to_move_to = self.subjects[0].choose_new_city_to_move_to(desperate=True)
                home_in_that_city = self.subjects[0].secure_home(city=new_city_to_move_to)
                return home_in_that_city
    ...
    def _move_spouses_and_any_kids_in_together(self, home_they_will_move_into):
        ...
        if home_they_will_move_into:
            ...
            spouse1.move(new_home=home_they_will_move_into, reason=self)
        else:
            # If that fails, have them find a new city to move to; let them be desperate,
            # if they have to (this ensures that they will for sure find a home to move to)
            new_city_to_move_to = spouse1.choose_new_city_to_move_to(desperate=True)
            spouse1.move_to_new_city(city=new_city_to_move_to, reason=self)
...
class Move(Event):
    ...
    def __init__(self, subjects, new_home, reason):
        ...
        super(Move, self).__init__(cosmos=subjects[0].cosmos)
        self.subjects = subjects
        ...
        for person in self.subjects:
            ...
            # Update resident listing for the new city, if applicable
            if person.city is not new_home.city:
                person.city = new_home.city
                new_home.city.residents.add(person)
            # Go to your new home
...
class NameChange(Event):
    ...
    def __init__(self, subject, new_last_name, reason, lawyer):
        ...
        super(NameChange, self).__init__(cosmos=subject.cosmos)
        self.city = subject.city
        self.subject = subject
