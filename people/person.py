...
class Person(object):
    ...
    def __init__(self, cosmos, birth):
        """Initialize a Person object."""
        # Set location and gameplay instance
        self.cosmos = cosmos
        self.id = self.cosmos.current_person_id
        self.cosmos.current_person_id += 1
        self.type = "person"
        self.birth = birth
        if birth:
            ...
            self.ready_to_work = (
                True if self.age >= self.cosmos.config.age_people_start_working(year=self.cosmos.year) else False
            )
        # Set sex
        ...
        self.departures = []  # Leaving a city
        self.death = None
        ...
        # Currently, whether a character is the player character is only considered by Conversation
    ...
    def __str__(self):
        ...
        if self.alive:
            return "{}, {} years old, {}".format(self.name, self.age, self.city.full_name)
        else:
            return "{}, {}-{}, {}".format(self.name, self.birth_year, self.death_year, self.city.full_name)
    ...
    def present(self, city):
        ...
        if self.alive and self.city is city:
            return True
        else:
            return False
    ...
    def next_of_kin(self):
        ...
        elif self.mother and self.mother.present(city=self.city):
            next_of_kin = self.mother
        elif self.father and self.father.present(city=self.city):
            next_of_kin = self.father
        elif any(k for k in self.kids if k.adult and k.present(city=self.city)):
            next_of_kin = next(k for k in self.kids if k.adult and k.present(city=self.city))
        elif any(f for f in self.siblings if f.adult and f.present(city=self.city)):
            next_of_kin = next(f for f in self.siblings if f.adult and f.present(city=self.city))
        elif any(f for f in self.extended_family if f.adult and f.present(city=self.city)):
            next_of_kin = next(f for f in self.extended_family if f.adult and f.present(city=self.city))
    ...
    def nuclear_family(self):
        ...
        for kid in self.spouse.kids & self.kids if self.spouse else self.kids:
            if kid.home is self.home and kid.present(city=self.city):
    ...
    def kids_at_home(self):
        ...
        kids_at_home = {k for k in self.kids if k.home is self.home and k.present(city=self.city)}
        return kids_at_home
    ...
    def change_name(self, new_last_name, reason):
        ...
        lawyer = self.contract_person_of_certain_occupation(
            city=self.city, occupation_in_question=occupation.Lawyer
        )
        if lawyer:
    ...
    def _determine_whether_pregnant(self, partner):
        ...
        if random.random() < chance_of_conception:
            ...
            female_partner.conception_year = self.cosmos.year
            female_partner.due_date = self.cosmos.ordinal_date + 270
    ...
    def divorce(self, partner):
        ...
        lawyer = self.contract_person_of_certain_occupation(city=self.city, occupation_in_question=occupation.Lawyer)
        lawyer.occupation.file_divorce(clients=(self, partner,))

    def give_birth(self):
        """Select a doctor and go to the hospital to give birth."""
        doctor = self.contract_person_of_certain_occupation(
            city=self.city, occupation_in_question=occupation.Doctor
        )
        if doctor:
    ...
    def die(self, cause_of_death):
        ...
        mortician = self.next_of_kin.contract_person_of_certain_occupation(
            city=self.city, occupation_in_question=occupation.Mortician
        )
        if mortician:
    ...
    def _find_job_at_the_family_company(self):
        ...
        for family_company in family_companies:
            ...
            # If its your parent, they will accommodate and add another supplemental position
                ...
                initial_job_vacancies = self.cosmos.config.initial_job_vacancies
                if self.cosmos.config.initial_job_vacancies[family_company.__class__]['day']:
                    position, shift = (
                        initial_job_vacancies[family_company.__class__]['day'][0], 'day'
                    )
                elif self.cosmos.config.initial_job_vacancies[family_company.__class__]['night']:
                    position, shift = (
                        initial_job_vacancies[family_company.__class__]['night'][0], 'night'
                    )
                elif self.cosmos.config.initial_job_vacancies[family_company.__class__]['supplemental day']:
                    position, shift = (
    ...
    def accept_job_offer(self, city, offered_job):
        """Decide whether to accept a job offer."""
        # TODO make this reasoning more interesting, e.g., consider the city it would be in
        level_of_offered_job = self.cosmos.config.job_levels[offered_job]
        if self.spouse and self.spouse.occupation:
            if self.spouse.occupation.level >= level_of_offered_job:
                return False
            # Don't let the spouse leave a job they started just now
            if self.spouse.occupation.years_experience < 1:
                return False
        return True

    def move_to_new_city(self, city, reason, forced_cohort=set()):
        """Secure housing in a new city and move there."""
        new_home = self.secure_home(city=city)
        # TODO PEOPLE ARE MOVING INTO OTHER PEOPLE'S HOMES AS DUCT TAPE
        if not new_home:
            someone_elses_home = random.choice(list(city.dwelling_places))
            self.move(new_home=someone_elses_home, reason=reason, forced_cohort=forced_cohort)
        else:
            self.move(new_home=new_home, reason=reason, forced_cohort=forced_cohort)

    def choose_new_city_to_move_to(self, desperate=False):
        """Consider moving to a new city."""
        city_to_move_to = None
        # First, look for a nearby underpopulated city
        if any(c for c in self.city.nearest_cities if c.underpopulated):
            nearby_underpopulated_cities = [c for c in self.city.nearest_cities if c.underpopulated]
            city_to_move_to = self._choose_a_city_to_move_to_from_a_pool_of_candidate_cities(
                pool=nearby_underpopulated_cities
            )
        # If that fails, look for an underpopulated city in your state
        if not city_to_move_to and any(c for c in self.city.state.cities if c.underpopulated):
            underpopulated_cities_in_my_state = [c for c in self.city.nearest_cities if c.underpopulated]
            city_to_move_to = self._choose_a_city_to_move_to_from_a_pool_of_candidate_cities(
                pool=underpopulated_cities_in_my_state
            )
        # If that also fails, scour the nation for an underpopulated city
        if not city_to_move_to and any(c for c in self.city.country.cities if c.underpopulated):
            underpopulated_cities_in_my_country = [c for c in self.city.country.cities if c.underpopulated]
            city_to_move_to = self._choose_a_city_to_move_to_from_a_pool_of_candidate_cities(
                pool=underpopulated_cities_in_my_country
            )
        # If that failed too and you're desperate, pick a random underpopulated city anywhere
        # in the cosmos
        if not city_to_move_to and any(c for c in self.cosmos.cities if c.underpopulated):
            underpopulated_cities_in_the_cosmos = [c for c in self.cosmos.cities if c.underpopulated]
            city_to_move_to = self._choose_a_city_to_move_to_from_a_pool_of_candidate_cities(
                pool=underpopulated_cities_in_the_cosmos
            )
        # Finally, if you still haven't found a city and you're desperate, pick a random city in the
        # cosmos, whether it's underpopulated or not
        if not city_to_move_to and desperate:
            city_to_move_to = self._choose_a_city_to_move_to_from_a_pool_of_candidate_cities(pool=[], desperate=True)
        return city_to_move_to

    def _choose_a_city_to_move_to_from_a_pool_of_candidate_cities(self, pool, desperate=False):
        """Choose a city to move from a pool of candidate cities."""
        # TODO make this reasoning more interesting
        # If you're desperate, just randomly choose a city somewhere in the cosmos
        if desperate:
            return random.choice(self.cosmos.cities)
        if any(c for c in pool if c.underpopulated):
            city_to_move_to = random.choice([c for c in pool if c.underpopulated])
        else:
            if (random.random() < 0.05 or desperate) and pool:
                city_to_move_to = random.choice(pool)
            else:
                city_to_move_to = None
        return city_to_move_to

    def move_out_of_parents_home(self):
        """Move out of parents' house, if possible."""
        home_to_move_into = self.secure_home(city=self.city)
        if home_to_move_into:
            self.move(new_home=home_to_move_into, reason=None)
        else:
            city_to_move_to = self.choose_new_city_to_move_to()
            if city_to_move_to:
                home_to_move_into = self.secure_home(city=self.city)
                if home_to_move_into:
                    self.move(new_home=home_to_move_into, reason=None)
    ...
    def move(self, new_home, reason, forced_cohort=set()):
        ...
        if forced_cohort:
            ...
            # this matter could be compromised by simply using nuclear_family
    ...
    def go_to(self, destination, occasion=None):
        ...
        if self.location:  # People just being instantiated won't have a location yet
            try:
                ...
                raise Exception(
                    "{} was not already in the people_here_now listing of {}, even though that's {} location".format(
                        self.name, self.location, self.possessive_pronoun
                    )
                )
    ...
    def contract_person_of_certain_occupation(self, city, occupation_in_question):
        ...
        """
        if city:
            pool = list(city.workers_of_trade(occupation_in_question))
        else:  # PersonExNihilo whose backstory is currently being retconned
            pool = []
        if pool:
    ...
    def _rate_potential_contractor_of_certain_occupation(self, person):
        ...
        for decision_maker in people_involved_in_this_decision:
            if person in decision_maker.immediate_family:
                score += decision_maker.cosmos.config.preference_to_contract_immediate_family
            elif person in decision_maker.extended_family:  # elif because immediate family is subset of extended family
                score += decision_maker.cosmos.config.preference_to_contract_extended_family
            if person in decision_maker.friends:
                score += decision_maker.cosmos.config.preference_to_contract_friend
            elif person in decision_maker.acquaintances:
                score += decision_maker.cosmos.config.preference_to_contract_acquaintance
            if person in decision_maker.enemies:
                score += decision_maker.cosmos.config.dispreference_to_hire_enemy
            if person in decision_maker.former_contractors:
                score += decision_maker.cosmos.config.preference_to_contract_former_contract
        # Multiply score according to this person's experience in this occupation
        score *= person.cosmos.config.function_to_derive_score_multiplier_bonus_for_experience(
    ...
    def _purchase_home(self, home):
        ...
        realtor = self.contract_person_of_certain_occupation(
            city=home.city, occupation_in_question=occupation.Realtor
        )
        if self.spouse:
    ...
    def secure_home(self, city):
        """Find a home in the given city to move into.
        ...
        """
        chosen_home_or_lot = self._choose_vacant_home_or_vacant_lot(city=city)
        if chosen_home_or_lot:
            if chosen_home_or_lot in city.vacant_lots:
                # A vacant lot was chosen, so build
                home_to_move_into = self._commission_construction_of_a_house(lot=chosen_home_or_lot)
            elif chosen_home_or_lot in city.vacant_homes:
            ...
            home_to_move_into = None  # The city is full; this will spark a departure to another city
    ...
    def _commission_construction_of_a_house(self, lot):
        ...
        architect = self.contract_person_of_certain_occupation(
            city=lot.city, occupation_in_question=occupation.Architect
        )
    ...
    def _choose_vacant_home_or_vacant_lot(self, city):
        ...
        """
        home_and_lot_scores = self._rate_all_vacant_homes_and_vacant_lots(city=city)
    ...
    def _rate_all_vacant_homes_and_vacant_lots(self, city):
        """Rate all vacant homes and vacant lots."""
        scores = {}
        for home in city.vacant_homes:
        ...
        for lot in city.vacant_lots:
            ...
            if self.spouse:
                ...
                spouse_score = 0
            scores[lot] = (
                (my_score + spouse_score) * self.cosmos.config.penalty_for_having_to_build_a_home_vs_buying_one
            )
        return scores
    ...
    def rate_potential_lot(self, lot):
        ...
        relatives_in_town = {f for f in self.extended_family if f.city is lot.city and f.home is not self.home}
        score = 0
        ...
        friends_in_town = {f for f in self.friends if f.city is lot.city and f.home is not self.home}
        ...
        if self.occupation and self.occupation.company.city is lot.city:
    ...
    def observe(self):
        ...
        for thing in {self.location} | self.location.people_here_now - {self}:
            if random.random() < self.cosmos.config.chance_someone_observes_nearby_entity:
...
class PersonExNihilo(Person):
    ...
    def __init__(self, cosmos, job_opportunity_impetus, spouse_already_generated, ethnicity=None):
        super(PersonExNihilo, self).__init__(cosmos, birth=None)
        ...
        if spouse_already_generated:
            ...
            if not self.cosmos.config.employable_as_a[job_opportunity_impetus](applicant=self):
        ...
        job_level = self.cosmos.config.job_levels[job_opportunity_impetus]
        ...
        try:
            cosmos.birthdays[self.birthday].add(self)
        except KeyError:
            cosmos.birthdays[self.birthday] = {self}
        ...
        self.first_name, self.middle_name, self.last_name, self.suffix = (
            self._init_name(ethnicity=ethnicity)
        )
        ...
        if (job_opportunity_impetus and
                job_opportunity_impetus in self.cosmos.config.occupations_requiring_college_degree):
    ...
    def _get_random_birthday(self):
        ...
        born on a leap year).
        """
        month, day, _ = self.cosmos.get_random_day_of_year(year=self.birth_year)
        birthday = (month, day)
        return birthday
    ...
    def _init_name(self, ethnicity):
        ...
        elif ethnicity == "German":
            last_name = Name(value=Names.a_german_surname(), progenitor=self, conceived_by=(), derived_from=())
        elif ethnicity == "Irish":
            last_name = Name(value=Names.an_irish_surname(), progenitor=self, conceived_by=(), derived_from=())
        elif ethnicity == "English":
            last_name = Name(value=Names.an_english_surname(), progenitor=self, conceived_by=(), derived_from=())
        elif ethnicity == "Scandinavian":
            last_name = Name(value=Names.a_scandinavian_surname(), progenitor=self, conceived_by=(), derived_from=())
        else:  # ethnicity == "French"
            last_name = Name(value=Names.a_scandinavian_surname(), progenitor=self, conceived_by=(), derived_from=())
        suffix = ''
    ...
    def _init_birth_year_of_the_founder(self):
        ...
        birth_year = self.cosmos.true_year - age_at_current_year_of_sim
        return birth_year
    ...
    def _init_money(self):
        """Determine how much money this person has to start with."""
        return self.cosmos.config.amount_of_money_generated_people_from_outside_city_start_with
    ...
    def _init_potentially_retcon_family(self, spouse_already_generated, job_opportunity_impetus):
        ...
        if not spouse_already_generated:
            chance_of_having_family = self.cosmos.config.chance_person_ex_nihilo_starts_with_family
    ...
    def _init_generate_family(self, job_opportunity_impetus):
        ...
        spouse = PersonExNihilo(
            cosmos=self.cosmos, job_opportunity_impetus=job_opportunity_impetus, spouse_already_generated=self
        )
        self._init_retcon_marriage(spouse=spouse)
        self._init_retcon_births_of_children()
        self.cosmos.year = self.cosmos.true_year
    ...
    def _init_retcon_marriage(self, spouse):
        ...
        if (
            ...
            marriage_date >= self.cosmos.true_year
        ):
            # If so, don't bother regenerating -- just set marriage year to last year and move on
            marriage_date = self.cosmos.true_year - 1
        self.cosmos.year = int(round(marriage_date))
        self.marry(spouse)
    ...
    def _init_retcon_births_of_children(self):
        ...
        for year in xrange(self.marriage.year, self.cosmos.true_year+1):
