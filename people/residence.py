...
class DwellingPlace(object):
    ...
    def __init__(self, lot, owners):
        ...
        """
        self.id = owners[0].cosmos.current_place_id
        owners[0].cosmos.current_place_id += 1
        self.type = "residence"
        self.city = lot.city
    ...
    def locked(self):
        ...
        if not self.owners:
            locked = True
        elif self.city.cosmos.time_of_day == 'day' and not self.people_here_now:
            # Randomly decide who was the last to leave this home today
            index_in_owners_of_last_to_leave = int(
                round(self.city.cosmos.random_number_this_timestep * len(self.owners))
            )
            index_in_owners_of_last_to_leave -= 1
            last_to_leave = list(self.owners)[index_in_owners_of_last_to_leave]
            if self.city.cosmos.random_number_this_timestep > last_to_leave.personality.neuroticism:
                locked = True
        elif self.city.cosmos.time_of_day == "night":
            most_neurotic_owner = max(self.owners, key=lambda o: o.personality.neuroticism)
            if self.city.cosmos.random_number_this_timestep > most_neurotic_owner.personality.neuroticism:
                locked = True
        return locked
...
class House(DwellingPlace):
    ...
    def get_bulldozed(self):
        """Raze this house (likely for the purpose of building an ApartmentComplex on its lot to spur population)."""
        self.lot.building = None
        self.city.dwelling_places.remove(self)
        self.city.former_dwelling_places.add(self)