...
class Relationship(object):
    ...
    def __init__(self, owner, subject, preceded_by):
        ...
        self.when_they_met = owner.cosmos.date
        self.first_met_str = '{date} at {location}'.format(
            date=owner.cosmos.year, location=owner.location.name
        )
        self.where_they_last_met = owner.location  # These change as appropriate
        self.when_they_last_met = owner.cosmos.date
        self.last_met_str_base = (
            '{date} at {location}'.format(date=owner.cosmos.year, location=owner.location.name),
            self.owner.cosmos.ordinal_date
        )
        ...
        if not preceded_by:
            ...
            self.trust = owner.cosmos.config.function_to_determine_trust_charge_boost(charge=self.charge)
    ...
    def _init_determine_charge_increment(self):
        ...
        """
        config = self.owner.cosmos.config
        charge_increment = (
            self.compatibility +
    ...
    def last_met_str(self):
        ...
        return "{} ({} days ago)".format(
            base_str, self.owner.cosmos.ordinal_date-ordinal_date_they_last_met
        )
...
class Acquaintance(Relationship):
    ...
    def __init__(self, owner, subject, preceded_by):
        ...
        owner.update_salience_of(
            subject, change=owner.cosmos.config.salience_increment_from_relationship_change['acquaintance']
        )
...
class Enmity(Relationship):
    ...
    def __init__(self, owner, subject, preceded_by):
        ...
        # are unidirectional)
        owner.update_salience_of(
            subject, change=owner.cosmos.config.salience_increment_from_relationship_change['enemy']
        )
...
class Friendship(Relationship):
    ...
    def __init__(self, owner, subject, preceded_by):
        ...
        # are unidirectional)
        owner.update_salience_of(
            subject, change=owner.cosmos.config.salience_increment_from_relationship_change['friend']
        )
...
class Romance(Relationship):
    ...
    def __init__(self, owner, subject, preceded_by):
        ...
        # are unidirectional)
        owner.update_salience_of(
            subject, change=owner.cosmos.config.salience_increment_from_relationship_change['romance']
        )
